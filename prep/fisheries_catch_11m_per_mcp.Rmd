...
Here I will select the smallest fleet only (11 m length), for further analyses
```{r}
catch18_prep <- catch2018 %>% 
  ...
  clean_names(.)  |> 
filter(lengdegruppe  == "Under 11 m")
```
...
#remove fleets over 11 m
temp_catch <- temp_catch |> 
  filter(Fleet_length == "Under 11m")
```
...
Deleting municipalities that are not in the study (Beiarn  and Guovdageaidnu-Kautokeino)
...
```{r}
#write.csv(fish_nor_prep2, here("prep", "output_data", "catch_total_11m_per_mcp.csv"), row.names = F)
```
