...
```
/Users/marinaespinasse/Documents/Marina/Research/github/nor-prep
...
plot_fishcatch05_prep2  <- plot_fishcatch05_prep %>% 
  filter(!is.na(total_catch_ton_coast)) %>% 
  mutate(catch_quantiles = cut(total_catch_ton_coast,
                               breaks = intervals05,
                               labels = labels05,
                               include.lowest = T)) 
...
intervals18 <-c(0, 8000, 16000, 24000)#intervals for year 2018 only
...
# Grey scale maps for publication
...
```{r}
ggsave("./figs/fish_catch_all_years_grey.tiff", main_map_all_grey, width = 18, height = 16, dpi = 300)
```


Same but for years 2005 and 2018
...
main_map05_grey <- base_map05_grey +
  geom_path(kommuner.pol, mapping = aes(x = long, y = lat, group = id), color = 'grey31',  size = .4) +    geom_sf_text(data = komnames_prep05, aes(X,Y, label= municip_number),fontface = "bold", size = 2, col="white") 
main_map05_grey
```
...
```{r fig.height=5, fig.width=4, dpi = 400}
grDevices::cairo_pdf("./figs/fisheries_catch_2005_2018_grey.pdf", width = 10, height = 12, fallback_resolution = 300)
