...
```{r}
seafood <- vroom::vroom(file.path("~/github/spfood/prep/output_data/spfood_allvars.csv"))
...
```{r}
spfoodkom<-read_csv(here("prep", "output_data", "spfood_allvars.csv"))
...
```{r}
spfoodkom_prep <- spfoodkom %>% 
  mutate(Komnum = replace(municip_number,
                          municip_number == 1903,
                          1901))

spfoodkom_fin <-allkommuner[allkommuner$KOMM %in% spfoodkom_prep$Komnum,] 
```


```{r }
world <- ne_countries(scale = "medium", returnclass = "sf")
kommuner.pol <- spTransform(spfoodkom_fin, CRS("+proj=longlat +ellps=GRS80"))
kommunersf <- sf::st_as_sf(spfoodkom_fin, coords= c("long", "lat"),crs = 4326, agr="identity")
...
In 2005, LL 59 municipalities had  catch registered, bus some catches are zero
...
plot_fishcatch18_prep <-  fishcatch18 %>% 
  ...
  st_as_sf(.)
```
In 2018, 59 municipalities had registered catch.
...
base_map05 <- ggplot() +
  ...
  scale_fill_manual(
    values = cols,
    name = "Fisheries catch range,  tons",
    guide = guide_legend(
...
base_map_all <- ggplot() +
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "Fisheries catches in 2005-2018"
  ) +
  facet_wrap(year ~.)
```
...
ggsave("./figs/fish_catch_all_years.tiff", main_map_all, width = 12, height = 14, dpi = 300)
