...
## Historgrams of the 75 selected municipalities
For the mixed models, I will use only 75 municipalities, that is, 1040 (that had at least 10 years of data). Let's see if histograms are different for these observations. I exlcude total fisheries, focus only on coatal (they are very similar anyway)

```{r}
mcp75 <- read_csv(here::here("prep", "output_data", "mcp_75selected.csv"))

myvars75 <- mcp75[,c("aqua_prod_ton",
                         "total_catch_ton_coast",
                          "distance",
                          "unemployed",
                       "percent_wforce",
                          "population",
...
par(mfrow = c(2,4))
hist <- map2(myvars75,varnames75, ~ hist(.x, main = .y, col = "seagreen", xlab = " ", ylab = ""))
```


Save this hist for supplementary material:
```{r}
tiff(file = "./figs/histograms.tiff",width = 8, height = 6, units = 'in', res = 300)
par(mfrow = c(2,4))
hist <- map2(myvars75,varnames75, ~ hist(.x, main = .y, col = "seagreen", xlab = " ", ylab = ""))
dev.off()
```
...
```{r, results='hide',fig.keep='all'}
par(mfrow = c(3,3))
map2(myvars75,varnames75, ~ boxplot(.x, main = .y, col = "seagreen", xlab = " "))
```


Let's take a look at the variable aqua_prop which is a proportion of aquaculture in the total seafood production and at the log of the fisheries catches.  icheck this for all municipalities.
```{r}
par(mfrow=c(1,3))
...
Now just make a nicer version of this plot. Also I use rank of the geographical distnace instead of distance. This plot is based on the selected municiplities (75) only

```{r}
mcp75_plots <- mcp75
colnames(mcp75_plots) <- c("Year", 
                            "municip_number", "municip", "Sea area",
                           "Population",
                           "Population growth",
                           "Number of unemployed",
                           "Percent in workforce",
                           "Distance south-north",
                           "Distance south-north (rank)",
                           "aqua_prod_ton",
                           "total_catch",
                           "Total coastal catch",
                             "totobs" )
...
MyX75 <- c("Year", "Sea area", "Population", "Population growth", "Number of unemployed",  "Percent in workforce", "Distance south-north (rank)")
aqua_covs <- MyMultipanel.ggp2(Z = mcp75_plots,
                  varx = MyX75,
                  vary = "aqua_prod_ton",
                  ylab = "Aquaculture production (ton)",
                  addSmoother = TRUE,
...
fish_aqua <-ggplot(
  data = seafood_prep,
  mapping = aes(x = total_catch_ton_coast, y = aqua_prod_ton)) +
  geom_point() +
  geom_smooth() +
  labs(x = "Fisheries landings (ton)",
...
```{r}
fish_covs <- MyMultipanel.ggp2(Z = mcp75_plots,
                  varx = MyX75,
                  vary = "Total coastal catch",
...
```{r}
mcp75_temp <- arrange(mcp75, year) 
  acf(mcp75_temp$aqua_prod_ton, na.action = na.pass)
...
```{r}
acf(mcp75_temp$total_catch_ton_coast, na.action = na.pass)
